...
class BowlsPage extends React.Component {
  ...
  renderPage() {
    const name = _.pluck(Breaks.collection.find({ name: 'Bowls' }).fetch(), 'name');
    const location = _.pluck(Breaks.collection.find({ name: 'Bowls' }).fetch(), 'location');
    const image = _.pluck(Breaks.collection.find({ name: 'Bowls' }).fetch(), 'image');
    const type = _.pluck(Breaks.collection.find({ name: 'Bowls' }).fetch(), 'type');
    const difficulty = _.pluck(Breaks.collection.find({ name: 'Bowls' }).fetch(), 'difficulty');
    const description = _.pluck(Breaks.collection.find({ name: 'Bowls' }).fetch(), 'description');
    return (
      <div>
        <br/>
        <Segment style={{ padding: '2em' }} vertical>
          <Header id="bowls-page" as='h3' style={{ fontSize: '4em', textAlign: 'center' }}>{name}</Header>
        </Segment>
